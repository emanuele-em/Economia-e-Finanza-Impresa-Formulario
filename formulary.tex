...
\subsection{Impatto della quotazione}

\subsubsection{Quotazione in borsa}

\subsubsection{Modello di Myers e Majluf}

\begin{definitions*}
  M & Manager\\
  w & fondi propri\\
  k & valore del nuovo progetto\\
  A & Valore atteso degli assets correnti\\
  a>0 & valore ex-post della realizzazione di A\\
  B & valore atteso del surplus del progetto\\
  b>0 & valore ex-post della realizzazione di B\\
  \text{assunzione } 1 & w<k (i fondi propri non sono sufficienti a finanziare il progetto)\\
  eq = k-w & valore delle azioni da emettere\\
  p & valore di mercato delle azioni vecchie (eq=0)\\
  p^{eq} & valore dopo l'emissione delle azioni vecchie (eq>0)\\
  \text(assunzione ) 2 & le nuove azioni non vengono sottoscritte da vecchi azionisti \\
  \mathbb{E}(A|M) & valore atteso di A data la scelta di M di non emettere\\
  \mathbb{E}(A+B|M^{eq}) & Valore atteso di A + B data la scelta di M di emettere nuove azioni\\
  (a+w) & Valore senza progetto
\end{definitions*}
\textbf{Caso \(H_0\), M non emette azioni}
\begin{align*}
  eq&=0\\
  v^0&=a+w\\
  b&=0\\
  p &= \text{Valore di mercato delle azioni in circolazione}
\end{align*}
\textbf{Caso \(H_1\), M emette azioni e finanzia il progetto}
\begin{align*}
  eq>0\\
  v^0=a+w+b+eq
\end{align*}
Quota vecchi azionisti:
\begin{equation*}
  \frac{p^{eq}}{(p^{eq}+eq)}
\end{equation*}
Valore vecchie azioni:
\begin{equation*}
  \frac{p^{eq}}{(p^{eq}+eq)} (a+w+b+eq)
\end{equation*}
M emette azioni se:
\begin{align*}
  &\frac{p^{eq}}{(p^{eq}+eq)}(a+w+b+eq)\geq a+w \rightarrow\\
  \rightarrow &b \geq \left(\frac{eq}{p^{eq}}\right) (a+w) - eq
\end{align*}
\textbf{Prezzo che azionisti sono disposti a pagare senza nuova emissione}
\begin{align*}
  b &< \left( \frac{eq}{p^{eq}} \right) (a+w) - eq\\
  (a+w) &> p^{eq} \left[ 1+\frac{b}{eq} \right]\\
  p&=w + \mathbb{E}(A|M)
\end{align*}
Prezzo che gli azionisti sono disposti a pagare se si emette:
\begin{align*}
  b &\geq \left( \frac{eq}{p^{eq}}(a+w) - eq \right)\\
  p^{eq} &= w + \mathbb{E}(A+B|M^{eq}) 
\end{align*}
Confrontando i risultati:
\begin{align*}
  a+w &> p^{eq}\left[ 1+\frac{b}{eq} \right] \implies\\
  \implies a+w &> p^{eq}
\end{align*}
In assenza del progetto si sa che \(a=\mathbb{E}(A|M)\), si sostituisce:
\begin{align*}
  \mathbb{E}(A|M)&>p^{eq}-w \implies\\
  \implies p-w &> p^{eq}-w \implies\\
  \implies p&>p^{eq}
\end{align*}

\subsection{Contendibilitá del controllo}

\subsubsection{Allocazione e trasferimento dei diritti di voto}

\subsubsection{Benefici privati del controllo}

\subsubsection{Trasferimenti del controllo}

\subsubsection{Regole interne ed esterne all'impresa}

\subsubsection{Struttura dei diritti di voto}
...
\begin{definitions*}
  v & Valore dell'impresa\\
  y & Valore pubblico\\
  b & Valore privato / benefici privati\\
  y > b & Manager capace\\
  y < b & manager incapace
\end{definitions*}

\textbf{Caso Dual Class: Incumbent Capace - Rivale Incapace}

\begin{minipage}[t]{.45\linewidth}
  \begin{align*}
    y^i &= 200\\
    b^i &= 0\\
    classe^i &= \frac{200}{2}=100
  \end{align*}
\end{minipage}
\hfill
\vline
...
\begin{minipage}[t]{.45\linewidth}
  \begin{align*}
    y^R &= 180\\
    b^R &= 15\\
    classe^R &= \frac{180}{2}=90
  \end{align*}
\end{minipage}

R per scalare offre 101 (non è ammessa offerta parziale), l'azionista con diritto di voto B ha le seguenti opzioni:
\begin{itemize}
  \item Non accetta, R perde: mantiene 100
  \item Non accetta, R vince: ottiene 90
  \item Accetta, R vince: ottiene 101
\end{itemize}
\(i\) potrá controbattere solo con un valore di 100 ma riesce a fermare la scalata. R ha una perdita di 90-101=-11 ma ha un beneficio di 15 che compensa, ci guadagna -11 + 15 = 4

\textbf{Caso Dual Class: Incumbent Incapace - Rivale Capace}
...
\begin{minipage}[t]{.45\linewidth}
  \begin{align*}
    y^R &= 220\\
    b^R &= 0\\
    classe^R &= \frac{200}{2}=100
  \end{align*}
\end{minipage}

R offre 110 ma \(i\) può controbattere con 111, 100 da \(y^i\) e 11 da \(b^i\)

\textbf{Caso One Vote One Share: Incumbent capace - Rivale Incapace}
...
R dovrebbe offrire più di 200 ottenendo così una perdita: \(201-180+15=-6\)

\textbf{Caso One Vote One Share: Incumbent Incapace - Rivale Capace}
...
R offre 220 ed effettua la scalata

\textbf{Caso Dual Class: Incumbent Incapace - Rivale Incapace}
...
\begin{minipage}[t]{.45\linewidth}
  \begin{align*}
    y^R &= 300\\
    b^R &= 3\\
    classe^R &= \frac{300}{2}=150
  \end{align*}
\end{minipage}

R offre 153 mentre \(i\) può offrire massimo 151, R vince con 152 e guadagna 1
\[
v = \underbrace{152}_{\text{Azioni A}}+\underbrace{150}_{\text{Azioni B}}=302  
\]

\textbf{Caso One Vote One Share: Incumbent Incapace - Rivale Incapace}
...
R offre 303 mentre \(i\) può offrire massimo 251, R può quindi offrire anche solo 301

\[ v = \underbrace{301}_{\text{Azioni}}<302 \]

\section{CORPORATE GOVERNANCE}
\subsection{Meccanismi di disciplina del manager}
\subsubsection{Origine, definizione e strumenti della corporate governance}
\subsubsection{Meccanismi esterni di corporate governance}
\subsubsection{Meccanismi interni di corporate governace}
\subsection{Tutela degli azionisti}
\subsubsection{Fallimenti e scandali negli USA}
\subsubsection{Fallimenti e scandali in Italia}
\subsubsection{Protezione degli azionisti}
\subsubsection{Separazione tra proprietá e controllo}
\subsubsection{Caso studio: Pirami societarie in Italia}
\subsection{Approccio Law And Finance}
\subsubsection{Approccio Law and Finance}
\subsubsection{Dati degli azionisti}
\subsubsection{Diritti dei creditori}
\subsubsection{Law Enforcement and Accounting Standard}
\subsubsection{Influenza del sistema legale sulla struttura finanziaria proprietaria}
\subsubsection{Influenza della protezione degli investitori sullo sviluppo economico}

\section{ECONOMETRIA}
\subsection{Introduzione all'econometria}
\subsubsection{Natura e scopo dell'econometria}
\subsubsection{Dati sperimentali e dati osservazionali}
\subsubsection{Distribuzione di un modello econometrico}
\subsubsection{Esempio di costruzione di un modello econometrico}
\begin{definitions*}
  Y_1 & Prima media\\
  Y_2 & Seconda media\\
  SE(Y_1 - Y_2) & Errore standard\\
  SE & Standard Error\\
  t & statistica
\end{definitions*}
\begin{align*}
  t &= \frac{\overline{Y_1}-\overline{Y_2}}{SE(\overline{Y_1}-\overline{Y_2})}\\
  SE(\overline{Y_1}-\overline{Y_2}) &= \sqrt{\frac{s_1^2}{n_1} + \frac{s_2^2}{n_2}}\\
  s_i^2 = \frac{1}{n_i-1}\times \sum_{j=1}^{n_i}(Y_j-\overline{Y_i})^2\\
  \text{Intervallo di confidenza } 95\%= (\overline{Y_1}-\overline{Y_2}) \pm 1.96 \times SE(\overline{Y_1}-\overline{Y_2})\\
  \text{Indice di correlazione } = \frac{\text{covarianza}}{(\text{varianza di X})\times (\text{varianza di Y})} 
\end{align*}

\subsection{Regressione lineare con un solo regressore}
\begin{definitions*}
  ESS & varaibilitá spiegata\\
  TSS & Variabilitá totale
\end{definitions*}
\begin{align*}
  Y_i=\beta_0+\beta_1X_i+u_i\\
  \beta_1=\frac{\Delta Y}{\Delta X}\\
  R^2=\frac{ESS}{TSS}=\frac{\sum_{i=1}^n(\widehat{Y_i}-\overline{\widehat{Y_i}})^2}{\sum_{i=1}^n(Y_i-\overline{Y_i})^2}
\end{align*}
\subsubsection{Il modello di regressione lineare}
\subsubsection{Metodo dei minimi quadrati ordinari (ols)}
\subsubsection{Misure di bontá della regressione campionaria}
\subsubsection{Assunzioni dei minimi quadrati}
\subsubsection{Distribuzione campionaria degli stimatori ols}
\subsubsection{Verifica d'ipotesi e intervalli di confidenza per \(\widehat{\beta_1}\)}
\subsubsection{La regressione quando la \(x\) è una variabile dummy}
\subsubsection{Vantaggi e svantaggi del metodo dei minimi quadrati ordinari}
\subsection{Regressione lineare con regressori multipli}
\subsubsection{Distorsione da variabili omesse}
\subsubsection{Causalitá e analisi di regressione}
\subsubsection{Modello di regressione multipla}
\subsubsection{Misure della bontá della regressione campionaria}
\subsubsection{Assunzione dei minimi quadrati per la regressione multipla}
\subsubsection{Verifica di ipotesi e intervalil di confidenza}
\subsubsection{Specificazione della regressione: come si fa?}
\subsection{Regressione non lineare}
\subsubsection{Concetti generali}
\subsubsection{Funzioni non lineari di un'unica variabile indipendente}
\subsubsection{Funzioni non lineari a due variabili: interazioni}
\subsection{Regressione con variabile dipendente binaria (dummy)}
\subsubsection{Modello lineare di probabilitá}
\subsubsection{Regressione PROBIT e LOGIT}
\subsubsection{Metodo della massima verosimiglianza}
\subsubsection{Applicazione ai dati Boston HMDA}
